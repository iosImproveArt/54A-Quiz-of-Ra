...
struct FactsViewQuizram54Ref: View {
    @State var timer: Timer?
    @State var selectedFact: FactQuizram54Ref? = facts.randomElement()!
            ...
            if let fact = selectedFact {
                Image("fact.bg")
                    ...
                    .overlay {
                        VStack {
                            Text(fact.title)
                                .withFontQuizram54Ref(size: 20, weight: .regular, color: .hexQuizram54Ref("F3D377"))
                                .padding(.top, 16)
                                .padding(.horizontal)
                            Spacer()
                            Text(fact.content)
                                .withFontQuizram54Ref(size: 19.5, weight: .thin)
                                .padding(.horizontal)
                                .padding(.vertical, 8)
                        }
                    }
                ...
                Button {
                    ...
                    Image("green.button.bg")
                        ...
                        .overlay {
                            Text("Read the full")
                                .withFontQuizram54Ref(size: 15.6, weight: .regular)
                        }
                }.padding(.bottom)
                ...
                Button {
                    ...
                    Image("button.bg")
                        ...
                        .overlay {
                            Text("NEXT")
                                .withFontQuizram54Ref(size: 14.6, weight: .regular)
                        }
                }
            }
    ...
    private var sheetView: some View {
        VStack {
            if let fact = selectedFact {
                Text(fact.title)
                    .withFontQuizram54Ref(size: 22, weight: .regular, color: .hexQuizram54Ref("F3D377"))
                    .padding(.top, 12)
                ...
                VStack(spacing: 0) {
                    Rectangle()
                        .frame(height: 2)
                        .foregroundColor(.hexQuizram54Ref("F3D377"))
                    
                    Rectangle()
                        .foregroundColor(.hexQuizram54Ref("402E25"))
                        .frame(height: 55)
                        .overlay {
                            Text("Close")
                                .withFontQuizram54Ref(size: 15.5, weight: .light)
                        }
                }.onTapGesture {
                    showSheet = false
                }
...
#Preview {
    ContentViewQuizram54Ref(showLoading: false, selectedTab: .facts)
}


struct FactQuizram54Ref {
    let title: String // (не більше 5 слів)
    let content: String // (великий опис факту)
}

fileprivate let facts: [FactQuizram54Ref] = [
    FactQuizram54Ref(
